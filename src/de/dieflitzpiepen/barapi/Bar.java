package de.dieflitzpiepen.barapi;
...
public class Bar implements Listener{
	...
	private static MiniGameAPI plugin;
	
	public Bar(MiniGameAPI plugin){
		this.plugin = plugin;
	}
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
			...
			public void run() {
				...
				Util.sendPacket(player, getDragon(player, "")
						.getDestroyPacket());
	...
	public static void setMessage(final Player player, String message,
			int seconds) {
		Validate.isTrue(seconds >= 0, "Seconds must be above 1 but was: ",
				seconds);
